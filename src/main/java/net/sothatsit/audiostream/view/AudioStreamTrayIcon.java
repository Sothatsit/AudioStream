...
public class AudioStreamTrayIcon {
    ...
    // TODO : Could be converted to LoopedThread
    private Thread thread;
    private boolean running = true;

    public AudioStreamTrayIcon(AudioStreamModel model) {
        this.model = model;
    ...
    private boolean isClientRunning() {
        return model.audioClientManager.hasClientsRunning();
    }

    private boolean isServerRunning() {
        return model.audioServer.isRunning().get();
    }
    ...
    private BufferedImage generateIcon() {
        ...
        graphics.setColor(isClientRunning() ? Color.BLACK : Color.GRAY);
        graphics.drawString("A", x, y);
        x += 64;

        graphics.setColor(isServerRunning() ? Color.BLACK : Color.GRAY);
